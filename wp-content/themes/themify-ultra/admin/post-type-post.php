...
function themify_default_post_layout( $data = array() ){
    ...
    /**
     * Enable infinite single posts
     */
    $pre = 'setting-infinite_single_posts';
    $infinite_posts_checked = themify_check( $pre ) ? 'checked="checked"': '';
    $output .= '<p><span class="label">' . __('Enable infinite scroll', 'themify') . '</span><label for="'.$pre.'"><input type="checkbox" id="'.$pre.'" name="'.$pre.'" '.$infinite_posts_checked.' /> ' . __('Enable infinite scroll on single post view', 'themify') . '</label></p>';
